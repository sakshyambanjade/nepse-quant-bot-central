...
def scrape_index_close(date_str: str = None):
    ...
    print(f"\n📊 Scraping NEPSE index close for {date_str}")
    ...
    try:
        ...
        if data and len(data) > 0:
            ...
            print(f"✅ Index close: {close_price}")
            ...
            close_file.write_text(json.dumps(close_data, indent=2))
            print(f"💾 Saved close price -> {close_file}")
        ...
        print(f"❌ Error scraping close price: {e}")
        return None
...
def store_close_in_db(date_str: str, close: float, open_p: float, high: float, low: float):
    ...
    try:
        ...
        conn.commit()
        conn.close()
        print(f"💾 Stored in database: index_close")
